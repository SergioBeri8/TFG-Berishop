                ...
                <div className="flex gap-2 mt-3 justify-center">
                  {imagenes.map((img, i) => (
                    <img key={i} src={img.url} alt={`foto ${i}`}
                      ...
                      className={`w-16 h-16 object-contain rounded-lg cursor-pointer bg-gray-50 border-2 transition ${imagenActual === i ? 'border-black' : 'border-transparent'}`} />
                  ))}
                </div>
              )}
            </div>
          ) : (
            <div className="w-full h-72 bg-gray-100 rounded-xl mb-6 flex items-center justify-center">
